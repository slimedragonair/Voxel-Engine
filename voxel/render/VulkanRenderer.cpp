...
void VulkanRenderer::initialize(const RendererConfig& config)
{
    ...
    // workloads (5000+ chunks at ~32 KB vertex + ~8 KB index each).
    constexpr VkDeviceSize kVertexArenaBytes = 256ULL * 1024ULL * 1024ULL; // 256 MB
    constexpr VkDeviceSize kIndexArenaBytes  = 64ULL  * 1024ULL * 1024ULL; // 64 MB
...
void VulkanRenderer::beginFrame()
{
    ...
    recordFrameCommands(frame.commandBuffer, imageIndex);
    ...
    check(vkQueueSubmit(graphicsQueue_, 1, &submitInfo, frame.inFlight), "Failed to submit clear commands");
    ...
    ++frameIndex_;
    // K: advance to the next per-frame slot. The CPU will keep going; the new
    // slot's fence may already be signaled (free), letting frame N+1 record
    // and submit immediately while N's GPU work is still running.
...
bool VulkanRenderer::uploadChunkMesh(world::ChunkCoord coord, const meshing::ChunkMesh& mesh)
{
    ...
    if (!uploadIntoBuffer(vertexArena_.buffer(), vertexSlice.offset, mesh.vertices.data(), vertexBytes)
        || !uploadIntoBuffer(indexArena_.buffer(), indexSlice.offset, mesh.indices.data(), indexBytes)) {
        vertexArena_.release(vertexSlice);
        indexArena_.release(indexSlice);
        Logger::warn("Chunk mesh upload queue full; mesh deferred by caller");
        return false;
    }
    ...
    for (std::size_t s = 0; s < mesh.sections.size(); ++s) {
        ...
        world::ChunkCoord sectionCoord{coord.x, coord.y * 2 + static_cast<std::int64_t>(s), coord.z};
        ...
        upsertSceneEntryForCoord(sectionCoord, entry);
        chunkToSections_.emplace(coord, sectionCoord);
    }
    ...
    // `uploadIntoBuffer` already adds to `frameStats_.stagingUploadBytes`.
...
void VulkanRenderer::createInstance(const RendererConfig& config)
{
    ...
    appInfo.pEngineName = "Voxel Engine Finale";
...
void VulkanRenderer::createIndirectResources()
{
    ...
    // Shared descriptor pool: kFramesInFlight × (1 chunk set + 1 cull set) + 1 material set.
    const std::uint32_t maxSets = kFramesInFlight * 2U + 1U;
...
{
    const auto range = chunkToSections_.equal_range(chunkCoord);
    for (auto it = range.first; it != range.second; ++it) {
        const auto indexIt = sceneEntryIndex_.find(it->second);
    ...
    chunkToSections_.erase(chunkCoord);
}
...
void VulkanRenderer::rebuildUiVertexBuffer(FrameContext& frame)
{
    ...
    const auto toNdcY = [this](float y) {
        return 1.0F - (y / static_cast<float>(swapchainExtent_.height)) * 2.0F;
    };
...
void VulkanRenderer::recordFrameCommands(VkCommandBuffer commandBuffer, std::uint32_t imageIndex)
{
    ...
    VkClearValue clear{};
    clear.color = {{0.04F, 0.07F, 0.09F, 1.0F}};
    ...
    if (graphicsPipeline_ != VK_NULL_HANDLE && !uploadedMeshes_.empty()
        ...
        if (computeAvailable) {
            ...
            vkCmdFillBuffer(commandBuffer, frame.indirectCommand.buffer, 0, VK_WHOLE_SIZE, 0);
