                    ...
                    const BlockLightCatalog& catalog, ChunkLightData& out)
{
    ...
    for (int z = 0; z < ChunkSize; ++z) {
        for (int x = 0; x < ChunkSize; ++x) {
            ...
            for (int y = ChunkSize - 1; y >= 0; --y) {
                ...
                // W0: graded attenuation. attenuation=15 fully blocks light;
                // partial values (water=1, leaves=2, ice=1) softly dim it as it
                // passes through, propagating the reduced level downward.
                if (info.attenuation >= 15U) {
                    level = 0U;
                    continue;
                }
                const std::uint8_t reduced = (level > info.attenuation)
                    ? static_cast<std::uint8_t>(level - info.attenuation)
                    : std::uint8_t{0};
                out.setSkyLight(x, y, z, reduced);
                if (reduced > 1U) {
                    skyFrontier.push_back(packCell(x, y, z, reduced));
                }
                level = reduced;
            }
        }
    ...
    if (chunks != nullptr) {
        const auto trySeedSky = [&](int x, int y, int z, std::uint8_t incoming) {
            ...
            // W0: incoming light loses 1 for the step + attenuation for the block.
            const std::uint8_t cost = static_cast<std::uint8_t>(1U + info.attenuation);
            if (incoming <= cost) {
                return;
            }
            const auto seeded = static_cast<std::uint8_t>(incoming - cost);
    ...
    for (std::size_t cursor = 0; cursor < skyFrontier.size(); ++cursor) {
        ...
        for (const auto& delta : kNeighbours) {
            ...
            // W0: graded attenuation — 1 for the BFS step + the block's own.
            const std::uint8_t cost = static_cast<std::uint8_t>(1U + info.attenuation);
            if (level <= cost) {
                continue;
            }
            const auto next = static_cast<std::uint8_t>(level - cost);
