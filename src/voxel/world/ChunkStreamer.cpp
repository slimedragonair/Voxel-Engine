...
std::vector<ChunkRequest> ChunkStreamer::planRequests(ChunkCoord center, const StreamingSettings& settings, core::Vec3 forward) const
{
    ...
    if (settings.pinnedVerticalChunkY.has_value()) {
        const int pinnedRadius = std::max(0, settings.pinnedVerticalChunkRadius);
        for (int bandY = *settings.pinnedVerticalChunkY - pinnedRadius;
             bandY <= *settings.pinnedVerticalChunkY + pinnedRadius;
             ++bandY) {
            if (bandY >= center.y - vRadius && bandY <= center.y + vRadius) {
                continue;
            }
            const float bandOffset = static_cast<float>(std::abs(bandY - *settings.pinnedVerticalChunkY));
            for (int dz = -hRadius; dz <= hRadius; ++dz) {
                for (int dx = -hRadius; dx <= hRadius; ++dx) {
                    const float horizontalDistance2 = static_cast<float>((dx * dx) + (dz * dz));
                    const float forwardDot = static_cast<float>(dx) * forwardX + static_cast<float>(dz) * forwardZ;
                    const float forwardBias = std::max(0.0F, forwardDot) * 0.25F;
                    const float priority = (horizontalDistance2 * 0.5F) + 1.0F + (bandOffset * 2.0F) - forwardBias;
                    requests.push_back({{center.x + dx, bandY, center.z + dz}, priority});
                }
            }
        }
    }
