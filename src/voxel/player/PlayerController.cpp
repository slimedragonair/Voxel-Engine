...
void PlayerController::setPosition(core::DVec3 position) noexcept
{
    position_ = position;
    velocity_ = {};
    grounded_ = false;
...
void PlayerController::tick(const world::ChunkManager& chunks, const PlayerInput& input, float deltaSeconds)
{
    ...
    if (noclip_) {
        ...
        const auto offset = fly * (speed * deltaSeconds);
        position_.x += static_cast<double>(offset.x);
        position_.y += static_cast<double>(offset.y);
        position_.z += static_cast<double>(offset.z);
        velocity_ = {};
        grounded_ = false;
        return;
    }

    velocity_.x = static_cast<double>(wish.x * speed);
    velocity_.z = static_cast<double>(wish.z * speed);
    if (input.jump && grounded_) {
        velocity_.y = static_cast<double>(config_.jumpVelocity);
        grounded_ = false;
    }
    velocity_.y -= static_cast<double>(config_.gravity * gravityScale_ * deltaSeconds);

    grounded_ = false;
    const double dt = static_cast<double>(deltaSeconds);
    moveAxis(chunks, position_.x, static_cast<float>(velocity_.x * dt), 0);
    moveAxis(chunks, position_.z, static_cast<float>(velocity_.z * dt), 2);
    moveAxis(chunks, position_.y, static_cast<float>(velocity_.y * dt), 1);
}
...
core::DVec3 PlayerController::dPosition() const noexcept
{
    return position_;
}
...
{
    return {static_cast<float>(velocity_.x), static_cast<float>(velocity_.y), static_cast<float>(velocity_.z)};
}

core::Vec3 PlayerController::eyePosition() const noexcept
{
    return {static_cast<float>(position_.x), static_cast<float>(position_.y + static_cast<double>(config_.eyeHeight)), static_cast<float>(position_.z)};
}

core::DVec3 PlayerController::dEyePosition() const noexcept
{
    return {position_.x, position_.y + static_cast<double>(config_.eyeHeight), position_.z};
}
...
bool PlayerController::overlapsBlock(world::ChunkCoord chunk, world::BlockCoord block) const noexcept
{
    ...
    const auto box = aabbAt(position());
...
{
    return core::normalize(core::Vec3{std::sin(yawRadians_), 0.0F, std::cos(yawRadians_)});
}

core::Vec3 PlayerController::rightFlat() const noexcept
{
    return core::normalize(core::cross(forwardFlat(), core::Vec3{0.0F, 1.0F, 0.0F}));
}
...
void PlayerController::moveAxis(const world::ChunkManager& chunks, double& component, float delta, int axis)
{
    ...
    const double original = component;
    component += static_cast<double>(delta);
    if (!collidesAt(chunks, position())) {
        return;
    }
    ...
    for (int i = 0; i < kCollisionSearchIterations; ++i) {
        ...
        component = original + (static_cast<double>(delta) * t);
        if (collidesAt(chunks, position())) {
            hi = t;
        } else {
            best = t;
    ...
    component = original + (static_cast<double>(delta) * best);
